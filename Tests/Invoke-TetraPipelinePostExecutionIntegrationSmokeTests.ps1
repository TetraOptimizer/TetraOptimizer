...
function Scan([string]$id,[string[]]$paths,[long]$free=1000,[string]$status='Complete'){
 ...
 [PSCustomObject]@{RecordType='SystemScanSnapshot';ScanId=$id;Status=$status;FileWorkRequested=$true;RootPaths=@('C:\Synthetic');Processes=@();Services=@();Startup=@();Applications=@();ScheduledTasks=@();Drivers=@();StorageVolumes=@([PSCustomObject]@{DeviceId='C:';FreeBytes=$free;UsedBytes=(10000-$free)});Files=@($files);Cleanup=@();Duplicates=@();AnalyzerState=@();Errors=@()}
}
$before=Scan 'before' @('C:\Synthetic\temp.tmp') 1000
$after=Scan 'after' @() 1100
$analysis=[PSCustomObject]@{RecordType='SystemAnalysisSnapshot';Findings=@()}
$recs=[PSCustomObject]@{RecordType='RecommendationSnapshot';Recommendations=@([PSCustomObject]@{RecommendationId='r1';Subject='temp.tmp';Recommendation='Cleanup';ProposedAction='CleanupFile';RequiresUserApproval=$true;Confidence='High';Reason='Synthetic'})}
...
$baseArgs=@{Profile='Balanced';RootPaths=@('C:\Synthetic');IncludeFileInventory=$true;Execute=$true;ScanProvider={ $before };AnalysisProvider={param($s,$p)$analysis};RecommendationProvider={param($a)$recs};ApprovalProvider={param($r)[PSCustomObject]@{ApprovedRecommendationIds=@('r1');DuplicateSelections=@{}}};ActionPlanProvider={param($r,$a)$plan};ExecutionProvider={param($p,$e)$execution};PostExecutionScanProvider={param($b,$e)$after}}
...
T 'Core six-stage contract remains intact' {Assert ($p.StageCount-eq6) "Expected 6 core stages; got $($p.StageCount)."}
...
T 'Deleted target is verified absent by integrated rescan' {Assert ($p.PostExecutionVerification.Counts.Verified-eq1) 'Expected one verified target.';Assert ($p.PostExecutionVerification.TargetVerifications[0].VerificationState-eq'Verified') 'Target not verified.'}
...
T 'Observed storage delta flows into final report without attribution' {Assert ($p.Report.After.ObservedVolumeDeltas.Count-eq1) 'Expected volume delta.';Assert ($p.Report.After.ObservedVolumeDeltas[0].FreeBytesDelta-eq100) 'Free delta mismatch.';Assert ($p.Report.After.ObservedVolumeDeltas[0].Attribution-eq'ObservedOnly') 'Delta was attributed.'}
...
T 'Rescan failure is honest and does not erase successful core execution' {$a=$baseArgs.Clone();$a.PostExecutionScanProvider={throw 'synthetic rescan failure'};$x=Invoke-TetraPipeline @a;Assert ($x.Status-eq'Complete') 'Core status was destroyed.';Assert ($x.LifecycleStatus-eq'CompletedWithPostExecutionIssues') 'Lifecycle issue not reported.';Assert ($x.PostExecutionRescanState-eq'Failed') 'Rescan failure not recorded.';Assert ($x.PostExecutionErrors.Count-ge1) 'Post error missing.'}
T 'Partial after-scan is preserved honestly in verification and report' {$partial=Scan 'partial-after' @() 1100 'Partial';$a=$baseArgs.Clone();$a.PostExecutionScanProvider={param($b,$e)$partial};$x=Invoke-TetraPipeline @a;Assert ($x.PostExecutionVerification.Status-eq'Partial') 'Partial verification not preserved.';Assert ($x.Report.After.AfterScanStatus-eq'Partial') 'Partial report after-state not preserved.'}
...
Write-Host '';Write-Host "PASS: $pass/12" -ForegroundColor $(if($fail-eq0){'Green'}else{'Yellow'});Write-Host "FAIL: $fail/12" -ForegroundColor $(if($fail-eq0){'Green'}else{'Red'});Write-Host "Overall: $(if($fail-eq0){'PASS'}else{'FAIL'})" -ForegroundColor $(if($fail-eq0){'Green'}else{'Red'});if($fail-gt0){exit 1}
