...
function New-ReportPipeline {
    param([string]$ExecutionState='ExecutedVerified',[bool]$ExecuteRequested=$true)
    ...
    return [PSCustomObject]@{RecordType='PipelineSnapshot';PipelineRunId='pipe-1';Profile='Balanced';Status='Complete';LifecycleStatus='Complete';ExecuteRequested=$ExecuteRequested;FailedStage='';ErrorMessage='';Scan=[PSCustomObject]@{ScanId='scan-1';Status='Complete';Processes=@();Services=@();Startup=@();Applications=@();ScheduledTasks=@();Drivers=@();StorageVolumes=@();Files=@();Cleanup=@([PSCustomObject]@{Name='cache.tmp'});Duplicates=@()};Analysis=[PSCustomObject]@{Findings=@($finding)};Recommendations=[PSCustomObject]@{Recommendations=@($rec)};ActionPlan=[PSCustomObject]@{Items=@($plan)};Execution=[PSCustomObject]@{Results=@($result)}}
}
...
Test-Case 'Failed execution never claims reclaimed bytes or deletion' {$r=New-TetraClientReport (New-ReportPipeline -ExecutionState ExecutionFailed -ExecuteRequested $true);Assert-Equal 0 $r.Summary.BytesReclaimed 'Failed execution must reclaim zero in report.';Assert-Equal 0 @($r.Actions[0].DeletedPaths).Count 'Failed execution must not claim deletion.';Assert-Equal 'CompletedWithExecutionIssues' $r.Status 'Issue status mismatch.'}
Test-Case 'Preview blocker does not become execution issue' {$p=New-ReportPipeline -ExecutionState Blocked -ExecuteRequested $false;$p.ActionPlan.Items[0].PlanState='AwaitingApproval';$r=New-TetraClientReport $p;Assert-Equal 'Complete' $r.Status 'Preview blocker must not downgrade report status.';Assert-Equal 0 $r.Summary.ExecutionIssues 'Preview blocker must not count as execution issue.';Assert-Equal 1 $r.Summary.Unresolved 'Awaiting approval must remain unresolved.'}
