...
function ConvertTo-TetraSystemScanAnalyzerState {
    ...
    foreach ($mapping in $mappings) {
        ...
        try {
            $converted = @(& $mapping.Function -InventoryRecords $mapping.Records)
            foreach ($observation in $converted) {
                if ($null -ne $observation) { $observations.Add($observation) }
            }
        }
        catch {
            $Errors.Add([PSCustomObject]@{
                Section      = "AnalyzerState/$($mapping.Section)"
                ErrorMessage = $_.Exception.Message
                ObservedUtc  = (Get-Date).ToUniversalTime().ToString('o')
            })
        }
...
function Invoke-TetraSystemScan {
    ...
    if ($fileWorkRequested -and (@($RootPaths).Count -eq 0)) {
    ...
    foreach ($group in @($duplicates)) {
        try { $potentialDuplicateReclaim += [long]$group.PotentialReclaimBytes } catch { }
    }
    ...
    $snapshot = [PSCustomObject]@{
        ...
        FileWorkRequested              = $fileWorkRequested
        RootPaths                      = @($RootPaths)
        Counts                         = $counts
        PotentialDuplicateReclaimBytes = $potentialDuplicateReclaim
        Processes                      = @($processes)
        Services                       = @($services)
        Startup                        = @($startup)
    ...
    if (Get-Command Write-TetraLog -ErrorAction SilentlyContinue) {
        try {
            Write-TetraLog -Level 'Info' -Module 'SystemScanEngine' -Action 'SystemScan' -Target $scanId -Result $status -Message "System scan completed with status '$status', $($errors.Count) error(s), and $($analyzerState.Count) Analyzer observation(s)." | Out-Null
        }
        catch { }
    }
...
# Public Functions:
#   - Invoke-TetraSystemScan
#   - ConvertTo-TetraSystemScanAnalyzerState
